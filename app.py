...
# CSS مخصص - LIGHT MODE PROFESSIONAL
st.markdown("""
<style>
    /* خلفية فاتحة */
    .stApp {
    ...
    .main-header {
        background: linear-gradient(135deg, #4A90A4 0%, #2E86AB 50%, #1B4965 100%);
        padding: 20px;
        border-radius: 12px;
        margin-bottom: 20px;
        text-align: center;
...
<div class="main-header">
    ...
    <p style="color: #E8F4F8; margin-top: 10px;">نظام التحليل الكمي المتقدم للأسواق المالية</p>
</div>
""", unsafe_allow_html=True)
...
with col1:
    st.metric("🕐 توقيت المغرب", tm_mar)

with col2:
    st.metric("📊 الجلسة الحالية", session_txt)

with col3:
    st.metric("📈 عدد الأصول", len(ASSETS))
...
if st.button("🚀 بدء التحليل", type="primary", use_container_width=True):
    ...
    if results:
        ...
        df_results = df_results.sort_values(by=['is_low', 'SCORE'], ascending=[True, False])
        ...
        with col1:
            st.metric("🟢 إشارات شراء", buy_count)
        with col2:
            st.metric("🔴 إشارات بيع", sell_count)
        with col3:
            st.metric("🔵 انتظار", wait_count)
        with col4:
            st.metric("⚠️ مصيدة", trap_count)
        with col5:
            st.metric("⚪ سيولة منخفضة", low_liq_count)
        ...
        if filter_option == "شراء فقط":
            df_display = df_display[df_display['SIGNAL_TYPE'] == 'buy']
        elif filter_option == "بيع فقط":
            df_display = df_display[df_display['SIGNAL_TYPE'] == 'sell']
        elif filter_option == "انتظار":
            df_display = df_display[df_display['SIGNAL_TYPE'] == 'wait']
        elif filter_option == "إشارات نشطة (شراء + بيع)":
            df_display = df_display[df_display['SIGNAL_TYPE'].isin(['buy', 'sell'])]
        ...
        # تلوين الجدول - LIGHT MODE COLORS
        ...
        styled_df = df_show.style.apply(highlight_signals, axis=1)
        st.dataframe(styled_df, use_container_width=True, height=600)
        
        # عرض تفاصيل الإشارات النشطة
        ...
        if not active_signals.empty:
            ...
            for _, row in active_signals.iterrows():
                with st.expander(f"{'🟢' if row['SIGNAL_TYPE'] == 'buy' else '🔴'} {row['ASSET']} - {row['SIGNAL']}"):
                    ...
                    with col1:
                        st.metric("سعر الدخول", row['ENTRY'])
                    with col2:
                        st.metric("الهدف (TP)", row['TP'])
                    with col3:
                        st.metric("وقف الخسارة (SL)", row['SL'])
                    with col4:
                        st.metric("الجودة", row['QUALITY'])
    ...
    with col2:
        st.markdown("**الفوركس:**")
        for asset in assets_list[3:11]:
            st.write(f"• {asset}")
    
    with col3:
        st.markdown("**المؤشرات والسلع:**")
        for asset in assets_list[11:]:
            st.write(f"• {asset}")

# Footer
st.markdown("---")
st.markdown("""
<div style="text-align: center; color: #495057; padding: 20px; background: #FFFFFF; border-radius: 10px; box-shadow: 0 2px 8px rgba(0,0,0,0.06);">
    <p>🏛️ QUANT-CORE ZOZO v14.6.5 | Powered by XGBoost & YFinance</p>
    <p style="color: #6C757D;">© 2024 - نظام التحليل الكمي المتقدم</p>
</div>
""", unsafe_allow_html=True)